...
let AI = true;
let str; // declared as global for whatsapp API access
...
function display() {
	...
	for (let one of present) {
		const isPresent = one.getAttribute('is-present') === 'true';
		const prefix = one.id.substring(0, 3) === '209' ? '' : 'LE';
		const name = one.getAttribute('name');
		const haveName = nameButton.checked;
		const entry = `${prefix}${one.id.substring(8, 10)}${haveName ? ` - ${name}` : ''}`;
